	...
	public function datatable_rates(Request $request, $id)
	{
		...
		if ($request->ajax()) 
		{
			$data = QueryBuilder::for(Mod_Company::class)
					...
					->join('master_product_api', 'master_product_api.id', '=', 'master_rates_api_customer.product_api_id')
					->select('company_name','master_company.customerno','master_rates_api_customer.product_api_id','master_product_api.product','master_rates_api_customer.rates',DB::raw('CASE WHEN billcycleid = 1 THEN "Per Hit" ELSE "Tiering" END AS billcycle'))
					->distinct()

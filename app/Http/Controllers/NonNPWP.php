...
class NonNPWP extends Controller
{
    public function index(Request $request)
    {
		if(Session::get('userid'))
		{
			...
			return view('home.reports.NonNPWP.index')->with($data);
		}
		else
		{
        ...
        if(Session::get('userid'))
		{
			...
			if ($request->ajax()) 
			{
                $data = DB::table('bs_postpaid')
                        ...
                        ->select('bs_postpaid.PERIOD', 'master_company.customerno', 'bs_postpaid.bsno', 'master_company.company_name', 'master_company.address_npwp', 'master_company.npwpno', DB::raw('FORMAT(bs_postpaid.TOTALAMOUNT,0) AS TOTALAMOUNT'), DB::raw('FORMAT(bs_postpaid.TOTALVAT,0) AS TOTALVAT'))
			...
			//return view('home.reports.NonNPWP.index');
		}
        else
        {
			header("cache-Control: no-store, no-cache, must-revalidate");
	...
	public function download(Request $request, $params)
    {
        if(Session::get('userid'))
		{
			...
			return Excel::download(new RptNonNPWP($periode, $data), 'Rpt Non NPWP DataWiz API '.$periode.'.xlsx');
		}
        else
        {
			header("cache-Control: no-store, no-cache, must-revalidate");

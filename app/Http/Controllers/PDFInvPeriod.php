...
use App\Models\M_BSP;
use App\Models\M_BSP_Detail;
...
class PDFInvPeriod extends Controller
{
    private $fpdf;
    ...
    public function index($ids)
    {
        ...
        {
			$datas = DB::table('trx_screen_no_h')
					->where('id', $ids)
					->select('id', 'customerno', DB::raw('DATE_FORMAT(DATE_ADD(created_at, INTERVAL +1 MONTH),"%Y%m") as period'), DB::raw('DATE_FORMAT(DATE_ADD(created_at, INTERVAL +1 MONTH),"%Y%m%d") as tanggal'))
					->get();
			...
			foreach ($datas as $user)
			{
				...
				$bs_period		= $user->period;
				$tanggal		= date('Ymd'); //$user->tanggal;
			}

			//$cek = M_BSP::query()->where('bs_period.PERIOD', $bs_period)->where('bs_period.TOTALUSAGE', '>', 0)->where('bs_period.CUSTOMERNO', $custno)->where('bs_period.TRXH_ID', $id)->select(DB::raw('COUNT(ID) as tot'))->get();
			//foreach ($cek as $hsl)
			//{
			//	$totalcek = $hsl->tot;
			//}
			...
			//{
			//	return back()
			//		->with('error','Data Invoice belum ada di BS !!!');
			//}
			...
			$bs = M_BSP::query()
					->where('bs_period.PERIOD', $bs_period)
					->where('bs_period.TOTALUSAGE', '>', 0)
					->where('bs_period.CUSTOMERNO', $custno)
					->where('bs_period.TRXH_ID', $id)
			...
			$file_name = 'INV_'.$custno.'_'.$tanggal.'.pdf';
    ...
    public function ChapterTitle($id,$custno,$period,$report_date,$customerno,$customername,$amountdue,$duedate,$bsno,$laststatementdate,$vatno,$billingaddress1,$billingaddress2,$billingaddress3,$billingaddress4,$billingaddress5,$zipcode,$phone1,$previousbalance,$balanceadj,$previouspay,$totrefund,$prevbalance,$charge,$monthly,$totalusage,$usageadj,$totdiscount,$beforevat,$totalvat,$totalamount) 
    {
        ...
        $query1 = DB::table('bs_detail_period')
                    ->where('CUSTOMERNO', $custno)
                    ->where('PERIOD', $period)
                    ->where('PRSS_ID', 17)
					->where('TRXH_ID', $id)
        ...
        if ($query1[0]->counts > 0)
        {
            ...
            foreach($bs_detail1 as $bsd_id1)
            {
                ...
                //$this->fpdf->Cell(100,4,$description,0,0,'L',0);
				$this->fpdf->MultiCell(100,4,$description,'J');
				//$this->fpdf->Cell(100,4,$description.' - '.$periodservice,0,0,'L',0);
                ...
                $this->fpdf->Cell(125,1,number_format("$totalamt"),0,1,'R',0);
                $this->fpdf->Cell(5,1,$sign,0,1,'L',0);
                $totalusage = $totalusage + $totalamt;
                $sign = '';
            }

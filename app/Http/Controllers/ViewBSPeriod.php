...
class ViewBSPeriod extends Controller
{
    public function bs(Request $request, $id)
    {
		if(Session::get('userid'))
		{
			if ($request->ajax()) 
			{
                $data = QueryBuilder::for(M_BSP::class)  //DB::table('bs')
                        ->where('master_company.id', $id)
						->join('master_company', 'master_company.customerno', '=', 'bs_period.CUSTOMERNO')
    ...
    public function pay(Request $request, $id)
    {
		if(Session::get('userid'))
		{
			if ($request->ajax()) 
			{
                $data = QueryBuilder::for(Mod_PaymentPeriod::class)  //DB::table('trans')
                        ->where('master_company.id', $id)
                        ...
                        ->select('trans.CUSTOMERNO', DB::raw('DATE_FORMAT(entrydate,"%Y-%m-%d") AS ENTRYDATE'), DB::raw('DATE_FORMAT(transactiondate,"%Y-%m-%d") AS TRANSDATE'), DB::raw('CASE WHEN TRANSACTIONCODE = "P" THEN "PAYMENT" WHEN TRANSACTIONCODE = "B" THEN "BALANCED ADJUSTMENT" WHEN TRANSACTIONCODE = "D" THEN "DISCOUNT" WHEN TRANSACTIONCODE = "U" THEN "USAGE ADJUSTMENT" WHEN TRANSACTIONCODE = "R" THEN "REFUND" END AS TRANSCODE'), DB::raw('CONCAT("Rp. ", FORMAT(amount,0)) AS AMOUNT'), 'trans.paymentcode', DB::raw('paymentmethod.PAYMENTMETHOD AS PAYMETHOD'), 'info',DB::raw('CASE WHEN receiptno IS NULL THEN "-" ELSE receiptno END AS RECEIPTNO'),'settlement_status')

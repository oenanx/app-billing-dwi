...
class Demography_Api_Prepaid extends Model
{
	protected $connection = 'mysql_4';
    protected $table = 'datawhiz_app.api_demography_prepaid';
    ...
    {
        return $query->where('datawhiz_app.api_demography_prepaid.nik', 'like', '%' . $search . '%')
                     ->orWhere('datawhiz_app.api_demography_prepaid.noapi_id', 'like', '%' . $search . '%')
					 ->orWhere('datawhiz_app.api_demography_prepaid.nama', 'like', '%' . $search . '%');
    }
}

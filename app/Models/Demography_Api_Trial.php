...
class Demography_Api_Trial extends Model
{
	protected $connection = 'mysql_4';
    protected $table = 'datawhiz_app.api_demography_trial';
    ...
    {
        return $query->where('datawhiz_app.api_demography_trial.nik', 'like', '%' . $search . '%')
                     ->orWhere('datawhiz_app.api_demography_trial.noapi_id', 'like', '%' . $search . '%')
					 ->orWhere('datawhiz_app.api_demography_trial.nama', 'like', '%' . $search . '%');
    }
}

...
class Demography_Verification_Api_Postpaid extends Model
{
	protected $connection = 'mysql_4';
    protected $table = 'datawhiz_app.api_demography_verification_postpaid';
    ...
    {
        return $query->where('datawhiz_app.api_demography_verification_postpaid.nik', 'like', '%' . $search . '%')
                     ->orWhere('datawhiz_app.api_demography_verification_postpaid.nama', 'like', '%' . $search . '%')
					 ->orWhere('datawhiz_app.api_demography_verification_postpaid.noapi_id', 'like', '%' . $search . '%');
    }
}

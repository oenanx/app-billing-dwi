...
class Demography_Verification_Api_Prepaid extends Model
{
	protected $connection = 'mysql_4';
    protected $table = 'datawhiz_app.api_demography_verification_prepaid';
    ...
    {
        return $query->where('datawhiz_app.api_demography_verification_prepaid.nik', 'like', '%' . $search . '%')
                     ->orWhere('datawhiz_app.api_demography_verification_prepaid.nama', 'like', '%' . $search . '%')
					 ->orWhere('datawhiz_app.api_demography_verification_prepaid.noapi_id', 'like', '%' . $search . '%');
    }
}

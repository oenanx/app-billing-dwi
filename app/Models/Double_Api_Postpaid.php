...
class Double_Api_Postpaid extends Model
{
	protected $connection = 'mysql_4';
    protected $table = 'api_double_postpaid';
    ...
    {
        return $query->where('datawhiz_app.api_double_postpaid.noapi_id', 'like', '%' . $search . '%')
                     ->orWhere('datawhiz_app.api_double_postpaid.phone_input', 'like', '%' . $search . '%')
					 ->orWhere('datawhiz_app.api_double_postpaid.phone', 'like', '%' . $search . '%');
    }
}

...
class Double_Api_Trial extends Model
{
	protected $connection = 'mysql_4';
    protected $table = 'api_double_trial';
    ...
    {
        return $query->where('datawhiz_app.api_double_trial.noapi_id', 'like', '%' . $search . '%')
                     ->orWhere('datawhiz_app.api_double_trial.phone_input', 'like', '%' . $search . '%')
					 ->orWhere('datawhiz_app.api_double_trial.phone', 'like', '%' . $search . '%');
    }
}

...
class IdMatch_Api_Postpaid extends Model
{
	protected $connection = 'mysql_4';
    protected $table = 'datawhiz_app.api_idmatch_postpaid';
    ...
    {
        return $query->where('datawhiz_app.api_idmatch_postpaid.nik', 'like', '%' . $search . '%')
                     ->orWhere('datawhiz_app.api_idmatch_postpaid.noapi_id', 'like', '%' . $search . '%')
					 ->orWhere('datawhiz_app.api_idmatch_postpaid.real_name', 'like', '%' . $search . '%');
    }
}

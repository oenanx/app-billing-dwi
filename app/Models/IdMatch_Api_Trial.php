...
class IdMatch_Api_Trial extends Model
{
	protected $connection = 'mysql_4';
    protected $table = 'datawhiz_app.api_idmatch_trial';
    ...
    {
        return $query->where('datawhiz_app.api_idmatch_trial.nik', 'like', '%' . $search . '%')
                     ->orWhere('datawhiz_app.api_idmatch_trial.noapi_id', 'like', '%' . $search . '%')
					 ->orWhere('datawhiz_app.api_idmatch_trial.real_name', 'like', '%' . $search . '%');
    }
}

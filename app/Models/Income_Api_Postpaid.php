...
class Income_Api_Postpaid extends Model
{
	protected $connection = 'mysql_4';
    protected $table = 'datawhiz_app.api_income_postpaid';
    ...
    {
        return $query->where('datawhiz_app.api_income_postpaid.nik', 'like', '%' . $search . '%')
                     ->orWhere('datawhiz_app.api_income_postpaid.noapi_id', 'like', '%' . $search . '%')
					 ->orWhere('datawhiz_app.api_income_postpaid.name', 'like', '%' . $search . '%');
    }
}

...
class Mod_Company extends Model
{
	...
	public static function Update_Cpy($editid,$data)
	{
		//print_r($data);
		//exit();
		DB::table('master_company')->where('id', $editid)->update($data);
		DB::connection('mysql_3')->table('master_company')->where('id', $editid)->update($data);
	}
    ...
    public function scopeGeneralSearch(Builder $query, string $search): Builder
    {
        return $query->where('email_billing', 'like', '%' . $search . '%')
                     ...
                     ->orWhere('master_company.customerno', 'like', '%' . $search . '%')

...
class PhoneHistory_Api_Trial extends Model
{
	protected $connection = 'mysql_4';
    protected $table = 'datawhiz_app.api_phonehistory_trial';
    ...
    {
        return $query->where('datawhiz_app.api_phonehistory_trial.phone_no', 'like', '%' . $search . '%')
                     ->orWhere('datawhiz_app.api_phonehistory_trial.phone_md5', 'like', '%' . $search . '%')
					 ->orWhere('datawhiz_app.api_phonehistory_trial.noapi_id', 'like', '%' . $search . '%');
    }
}

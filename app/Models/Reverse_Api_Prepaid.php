...
class Reverse_Api_Prepaid extends Model
{
	protected $connection = 'mysql_4';
    protected $table = 'api_reverse_prepaid';
    ...
    {
        return $query->where('datawhiz_app.api_reverse_prepaid.noapi_id', 'like', '%' . $search . '%')
                     ->orWhere('datawhiz_app.api_reverse_prepaid.nik', 'like', '%' . $search . '%')
					 ->orWhere('datawhiz_app.api_reverse_prepaid.phoneno', 'like', '%' . $search . '%');
    }
}

...
class Reverse_Api_Trial extends Model
{
	protected $connection = 'mysql_4';
    protected $table = 'api_reverse_trial';
    ...
    {
        return $query->where('datawhiz_app.api_reverse_trial.noapi_id', 'like', '%' . $search . '%')
                     ->orWhere('datawhiz_app.api_reverse_trial.nik', 'like', '%' . $search . '%')
					 ->orWhere('datawhiz_app.api_reverse_trial.phoneno', 'like', '%' . $search . '%');
    }
}

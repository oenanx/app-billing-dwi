...
class Slik_Summary_Api_Prepaid extends Model
{
	protected $connection = 'mysql_4';
    protected $table = 'datawhiz_app.api_slik_summary_prepaid';
    ...
    {
        return $query->where('datawhiz_app.api_slik_summary_prepaid.nik', 'like', '%' . $search . '%')
                     ->orWhere('datawhiz_app.api_slik_summary_prepaid.noapi_id', 'like', '%' . $search . '%')
					 ->orWhere('datawhiz_app.api_slik_summary_prepaid.name', 'like', '%' . $search . '%');
    }
}

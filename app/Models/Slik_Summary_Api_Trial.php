...
class Slik_Summary_Api_Trial extends Model
{
	protected $connection = 'mysql_4';
    protected $table = 'datawhiz_app.api_slik_summary_trial';
    ...
    {
        return $query->where('datawhiz_app.api_slik_summary_trial.nik', 'like', '%' . $search . '%')
                     ->orWhere('datawhiz_app.api_slik_summary_trial.noapi_id', 'like', '%' . $search . '%')
					 ->orWhere('datawhiz_app.api_slik_summary_trial.name', 'like', '%' . $search . '%');
    }
}

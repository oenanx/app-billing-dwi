...
class Valid_No_Api_Postpaid extends Model
{
	protected $connection = 'mysql_4';
    protected $table = 'datawhiz_app.api_valid_no_postpaid';
    ...
    {
        return $query->where('datawhiz_app.api_valid_no_postpaid.phone_number', 'like', '%' . $search . '%')
                     ->orWhere('datawhiz_app.api_valid_no_postpaid.success', 'like', '%' . $search . '%')
					 ->orWhere('datawhiz_app.api_valid_no_postpaid.noapi_id', 'like', '%' . $search . '%');
    }
}

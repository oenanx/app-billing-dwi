...
class Valid_No_Api_Trial extends Model
{
	protected $connection = 'mysql_4';
    protected $table = 'datawhiz_app.api_valid_no_trial';
    ...
    {
        return $query->where('datawhiz_app.api_valid_no_trial.phone_number', 'like', '%' . $search . '%')
                     ->orWhere('datawhiz_app.api_valid_no_trial.success', 'like', '%' . $search . '%')
					 ->orWhere('datawhiz_app.api_valid_no_trial.noapi_id', 'like', '%' . $search . '%');
    }
}

...
@section('pageTitle')
	<h1 class="text-dark font-weight-bold my-1 mr-1">Maintenance Postpaid</h1>
    &nbsp;&nbsp;&nbsp;<ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-md">
		...
		<li class="breadcrumb-item">
			<a href="" class="text-muted">Maintenance Postpaid</a>
		</li>
	</ul>
@endsection
...
<div class="card card-custom" data-card="true" id="kt_card_1">
	<div class="card-header">
		<h3 class="card-title"><i class="far fa-copy"></i>&nbsp;&nbsp;Get Usage API Postpaid From Web Dashboard</h3>
	</div>
	...
	@csrf		
		<div class="card-body">
			<div class="row align-items-center" style="width:100%; font-size:10pt; height: auto;">
				...
				<div class="col-lg-6 col-lg-6">
					<div class="form-group">
						<input type="text" class="form-control input-sm" id="tahun" name="tahun" maxlength="4" required value="{{ strftime('%Y',strtotime('now')); }}" />
					</div>
				</div>
			</div>
		</div>
		<div class="card-footer">
			<div class="row align-items-right" style="width:100%; font-size:10pt; height: auto;">
				<div class="col-lg-12 col-lg-12">
					<button type="button" style="float: right;" class="btn btn-primary btn-md" id="Get_Usage" name="Get_Usage">
						<span class="svg-icon svg-icon-primary svg-icon-1x">
							<i class="flaticon2-accept icon-md"></i>&nbsp;Get Usage
						</span>
					</button>&nbsp;&nbsp;&nbsp;&nbsp;
					<button type="button" style="float: right;" name="Batal" class="btn btn-md btn-danger" id="Refresh" name="Refresh">
						<span class="svg-icon svg-icon-primary svg-icon-1x">
...
<div class="card card-custom" data-card="true" id="kt_card_2">
	<div class="card-header">
		<h3 class="card-title"><i class="fa fa-wrench"></i>&nbsp;&nbsp;Maintenance Billing Statement Postpaid</h3>
	</div>
			...
			<div class="row align-items-right" style="width:100%; font-size:10pt; height: auto;">
				<div class="col-lg-12 col-lg-12">
					<button type="button" style="float: right;" class="btn btn-primary btn-md" id="Simpan" name="Simpan">
						<span class="svg-icon svg-icon-primary svg-icon-1x">
							<i class="flaticon2-accept icon-md"></i>&nbsp;Process
						</span>
					</button>&nbsp;&nbsp;&nbsp;&nbsp;
...
{
	$('#Get_Usage').on("click", function ()
	{
        if (document.getElementById("months").value.trim() == "")
        {
        ...
        if (document.getElementById("tahun").value.trim() == "")
        {
		...
		$.ajax({
			url : "{{ url('MaintenancePostpaid/get_usage') }}",
			cache: false,
			contentType: false,
			processData: false,
			method : "POST",
			...
			success: function(data)
			{
				...
				if (data.success)
				{
					...
					$("#notif").html('<div class="box no-border"><div class="alert alert-success alert-dismissable"><i class="fa fa-check"></i><a href="{{ url('MaintenancePostpaid/index') }}"><button type="button" class="close"><i class="ki ki-close"></i></button></a><h4> Get usage semua customer sudah selesai !<br /> Proses memakan waktu : '+data.success+' detik </h4></div></div>').show();
	...
	$('#Simpan').on("click", function ()
	{
		...
		$.ajax({
			url : "{{ url('MaintenancePostpaid/proses') }}",
			cache: false,
			contentType: false,
			processData: false,
			method : "POST",
			...
			success: function(data)
			{
				...
				if (data.success)
				{
					...
					$("#notif").html('<div class="box no-border"><div class="alert alert-success alert-dismissable"><i class="fa fa-check"></i><a href="{{ url('MaintenancePostpaid/index') }}"><button type="button" class="close"><i class="ki ki-close"></i></button></a><h4> Data usage semua customer sudah di-maintenance !<br /> Proses memakan waktu : '+data.success+' detik </h4></div></div>').show();
					setTimeout(function () { $('#notif').hide(); }, 6000);
	...
	{
		var url = "{{ url('MaintenancePostpaid/index') }}";

...
@section('pageTitle')
	<h1 class="text-dark font-weight-bold my-1 mr-5">List of Users Customers DataWiz</h1>
    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
		...
		<li class="breadcrumb-item">
			<a href="javascript:;" class="text-muted">3. Form Users</a>
		</li>
		<li class="breadcrumb-item">
			<a href="{{ url('M_UserApi/index') }}" class="text-muted">List of Users Customers DataWiz</a>
		</li>
	</ul>
@endsection
...
<div class="card card-custom" data-card="true" id="kt_card_2">
	<div class="card-header">
		<div class="card-title">
			<h3 class="card-label"><i class="fa fa-th-list"></i> List of Users Customers DataWiz</h3>
		</div>
...
<div id="view-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered modal-xl">
		<div class="modal-content">
			<div class="card">
				<div class="card-header">
					<h1 class="card-title"><b>View Detail Users Media Customer</b></h1>
				</div>
				<div class="card-body">
					...
					<form enctype="multipart/form-data" id="form2" class="form-horizontal">
						...
						<table style="width:100%; font-size:11pt;" border="0">
							...
							<tr style="line-height: 1.0;">
								...
								<td align="left" style="width:22%;"> Transfer Media&nbsp;<label style="color: red;"><b>*</b></label></td>
								<td align="center" style="width:3%;">:</td>
								<td align="left" style="width:75%;"> 
...
<div id="view-modal-ftp" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered modal-xl">
		<div class="modal-content">
			<div class="card">
				<div class="card-header">
					<h2 class="card-title"><b>Edit Detail Dashboard</b></h2>
				</div>
				<div class="card-body">
					<div id="modal-loader" style="display: none; text-align: center;">
						<img src="{{ asset('assets/images/ajax-loader.gif') }}">
...
$(document).ready(function() 
{
    ...
    dataTable = $('#Show-Tables').KTDatatable(
	{
		...
		//},
		{
			...
			sortable: true,
			width: 260,
			title: '<p style="font-size:12px;">PARENT CUSTOMER</p>',
			template: function(row) {
				return '<p style="font-size:11px;">'+row.PARENT_CUSTOMER+'</p>';
			}
		},
		{
			field: 'username',
			...
			width: 190,
            textAlign: 'center',
			title: '<p style="font-size:12px;">User Name</p>',
			template: function(row) {
				return '<p style="font-size:11px;">'+row.username+'</p>';
			}
		},
		{
			field: 'apptype',
			...
			width: 260,
            textAlign: 'center',
			title: '<p style="font-size:12px;">Media - Product Type</p>',
            template: function(row) {
				return '<p style="font-size:11px;">'+row.apptype+'</p>';
            }
		},
		{
			field: 'Actions',
			...
			template: function(row) 
			{
				if (row.fsync == "2" && row.fftp == "0")
				{
					return '<div class="btn-group">\
							<button type="button" class="btn btn-icon btn-sm btn-warning btn-hover-light-primary" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="ki ki-arrow-down icon-sm"></i>\
							</button>\
							<ul class="dropdown-menu dropdown-menu-right" role="menu" x-placement="bottom-start">\
								<li style="font-size:9pt;>\
									<a href="javascript:void(0)" class="dropdown-item viewCpy" data-id="'+row.idx+'" title="View Details">\
								...
								<li style="font-size:9pt;>\
									<a href="javascript:void(0)" class="dropdown-item synch" data-id="'+row.idx+'" title="Sync to AppGlobal">\
										<span class="svg-icon svg-icon-primary svg-icon-2x">\
											<i class="flaticon-upload icon-md"></i>\
										</span>&nbsp;&nbsp;Synch to AppGlobal\
									</a>\
								</li>\
							</ul>\
						</div>';
				...
				{
					return '<div class="btn-group">\
							<button type="button" class="btn btn-icon btn-sm btn-primary btn-hover-light-primary" data-toggle="dropdown"  aria-haspopup="true" aria-expanded="false"><i class="ki ki-arrow-down icon-sm"></i>\
							</button>\
							<ul class="dropdown-menu dropdown-menu-right" role="menu" x-placement="bottom-start">\
								<li style="font-size:9pt;>\
									<a href="javascript:void(0)" class="dropdown-item viewCpy" data-id="'+row.idx+'" title="View Details">\
	...
	{     
		...
		$.ajax(
		{
			...
			success: function (data) 
			{
				...
				{
					console.log(data);
					$("#notif").html('<div class="alert alert-danger alert-dismissable"><i class="icon fa fa-ban"></i><button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button><h4> Ada kesalahan, Harap edit non sftp / ftp di sub menu Company dahulu... !!!</h4></div>').show();
					setTimeout(function () { $('#notif').hide(); }, 6000);
					dataTable.reload();
...
{     
	...
	$.get("{{ url('M_Users/view_user') }}"+'/'+id, function (data) 
	{
		...
		if (data.fftpdesc == "SFTP / FTP")
		{
			$('[name="ip_ftp1"]').val(data.ip_ftp);
			$('[name="username1"]').val(data.usernamesftp);
			$('[name="passwd1"]').val(data.passwdsftp);
		...
		else if (data.fftpdesc == "Dashboard")
		{
			...
			$('[name="status3a"]').val(data.fsyncdesc);
			$("#hilangb1").hide();
			$("#hilangb2").hide();
			$("#hilangb3").hide();
			$("#hilangb4").hide();

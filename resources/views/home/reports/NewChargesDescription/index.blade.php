...
@section('pageTitle')
	<h1 class="text-dark font-weight-bold my-1 mr-5">Report New Charges Description</h1>
    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
		...
		<li class="breadcrumb-item">
			<a href="{{ url('NewChargesDescription/index') }}" class="text-muted">Report New Charges Description</a>
		</li>
	</ul>
@endsection
...
<div class="card card-custom" data-card="true" id="kt_card_2">
	<div class="card-header">
		<div class="card-title">
			<h3 class="card-label"><i class="fa fa-money"></i> Report New Charges Description</h3>
		</div>
	...
	<div class="card-body">
		...
		<!--<div class="table table-bordered table-hover" style="width:100%; font-size:7pt;" width="100%">
			<table class="display" id="Show-Tables" style="width:100%; font-size:7pt;" width="100%">
				<thead align="center">
					<tr>
						<th><center>PERIOD</center></th>
						<th><center>CUSTOMERNO</center></th>
						<th><center>CUSTOMERNAME</center></th>
						<th><center>DESCRIPTION</center></th>
						<th><center>AMOUNT</center></th>
						<th><center>SALESAGENTNAME</center></th>
						<th><center>STATUSCODE</center></th>
						<th><center>ACTIVATIONDATE</center></th>
					</tr>
				</thead>
			</table>
		</div>
...
$(document).ready(function() 
{
	...
	$('#showdata').on("click", function ()
	{
		...
		dataTable = $('#Show-Tables').DataTable(
		{
			...
			ajax: "{{ url('NewChargesDescription/datatable') }}"+'/'+params,
			columns: [
				{data: 'PERIOD', className: 'text-center', name: 'PERIOD'},
				{data: 'CUSTOMERNO', className: 'text-left', name: 'CUSTOMERNO'},
				{data: 'company_name', className: 'text-left', name: 'company_name'},
				{data: 'DESCRIPTION', className: 'text-left', name: 'DESCRIPTION'},
				{data: 'AMOUNT', className: 'text-right', name: 'AMOUNT'},
				{data: 'SALESAGENTNAME', className: 'text-center', name: 'SALESAGENTNAME'},
				{data: 'active', className: 'text-center', name: 'active'},
				{data: 'activation_date', className: 'text-center', name: 'activation_date'},
			],
			order: [[ 1,3, "desc" ]],
            deferRender: true,
			dom: 'Blrtip',
		...
		dataTable = $('#Show-Tables').KTDatatable(
		{
			...
			data: 	
			{
				...
				source: {
					read: {
						//url: link,
						url: '{{ url('NewChargesDescription/datatable') }}'+'/'+params,
			...
			},
			{
				...
				sortable: false,
				width: 180,
				title: '<p style="font-size:10px;">DESCRIPTION</p>',
				template: function(data) {
					return '<p style="font-size:10px;">'+data.DESCRIPTION+'</p>';
				}
			},
			{
				field: 'AMOUNT',
				...
				sortable: false,
				width: 80,
				title: '<p style="font-size:10px;">AMOUNT</p>',
				template: function(data) {
					return '<p style="font-size:10px; text-align: right;">'+data.AMOUNT+'</p>';
				}
			},
			{
			...
			},
			{
				...
				sortable: false,
				width: 80,
				title: '<p style="font-size:10px;">STATUSCODE</p>',
				template: function(data) {
					return '<p style="font-size:10px;">'+data.active+'</p>';
				}
			},
			{
	...
	$('#downloaddata').on("click", function ()
	{
		...
		var params = thn+''+bln;
		//console.log(params);
		var url = "{{ url('NewChargesDescription/download') }}"+'/'+params;

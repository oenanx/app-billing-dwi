...
@section('pageTitle')
    ...
    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
		...
		<li class="breadcrumb-item">
			<a href="javascript:;" class="text-muted">View Usage API Product Customer</a>
		</li>
	</ul>
@endsection
...
$(document).ready(function() 
{
			...
			dataTable = $('#Show-TablesAll').KTDatatable(
			{
				...
				data: 	
				{
					...
					source: {
						read: {
							//url: link,
							url: '{{ url('ViewUsageAPI/datatableAll') }}'+'/'+params,
				...
				},
				{
					...
					sortable: false,
					width: 120,
					title: '<p style="font-size:12px; text-align: center;">PERIODE</p>',
					template: function(data) {
				...
				},
				{
					...
					template: function(data) {
						return '<p style="font-size:12px;">'+data.billingtype+'</p>';
					}
				},
				{
					field: 'tot_sukses',
					...
					sortable: false,
					width: 120,
					title: '<p style="font-size:12px; text-align: center;">SUCCESS HITS</p>',
					template: function(data) {
				...
				},
				{
					...
					sortable: false,
					width: 120,
					title: '<p style="font-size:12px; text-align: center;">FAILED HITS</p>',
					template: function(data) {
			...
			dataTable = $('#Show-Tables').KTDatatable(
			{
				...
				data: 	
				{
					...
					source: {
						read: {
							//url: link,
							url: '{{ url('ViewUsageAPI/datatableusage') }}'+'/'+params,
							// sample custom headers
							// headers: {'x-my-custom-header': 'some value', 'x-test-header': 'the value'},
							map: function(raw) {
								// sample data mapping
	...
	$('#downloaddata').on("click", function ()
	{
		...
		var params = thn+''+bln+';'+prd+';'+customerno;
		//console.log(params);
		var url = "{{ url('ViewUsageAPI/downloadlog') }}"+'/'+params;
